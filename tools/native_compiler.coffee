...
# :visitor should have `pre` and `post` properties, which should be dicts with
# node typenames as keys.
...
# identifiers and literals are often found doing the same thing (e.g. foo.bar
# and foo["bar"], so this provides a quick way to extract the underlying value
...
class ClassResolver
  constructor: (package_name) ->
    @package_name = util.int_classname package_name
    @class_name = ""
    @import_map = {}
  ...
  resolve: (shortname) ->
    ...
    if shortname of @import_map
      return @import_map[shortname]

    packages = [ "java/lang", @package_name ]
    for p in packages
      cls = "#{p}/#{shortname}"
      if (classfile = jvm.read_classfile cls)
        return { name: cls, classfile: classfile }
...
class Signature
  ...
  constructor: (@full_class_name, sig, @resolver) ->
    ...
    groups || throw new Error "Failed to parse signature #{sig}"
    keywords = if groups[1]? then groups[1].split /\s+/ else []
    @static = 'static' in keywords
    @nonnative = 'nonnative' in keywords
    @ret_type = groups[2]
    @name = groups[3]
    @args =
      if groups[4] is ''
      ...
      switch obj.callee.name
        ...
        when '_class'
          ...
          for prop in arg.right.properties
            ...
            unless m.access_flags.native or sig.nonnative
              throw new CompileError prop, "Trapping non-native method #{sig.toFullString()} without `nonnative` keyword"
            prop.key.value = sig.toFullString()
            continue if prop.value.type is 'Identifier' # defined as some util function elsewhere
...
compile = (file_list) ->
  ...
  for file in file_list
    ...
    # NodeVisitor should have deleted all the defined natives, having collated
    # them internally. The remaining statements should be globals like util
    # functions, which we collate separately.
